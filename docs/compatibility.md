...
| モデル select | 🟡 部分互換 | `where` (スカラー比較 `_eq` `_neq` `_gt` `_gte` `_lt` `_lte` `_in` `_like` `_is_null` と `_and` / `_or` / `_not`)、`order_by` (V3 準拠: 方向は `Asc` / `Desc` の 2 値、null 配置固定、1 エントリ 1 キーで複数キーはエラー)、`limit` / `offset`、エイリアス・複数ルートフィールド | テキスト検索演算子は `_like` のみ (`_ilike` `_regex` `_similar` 等は未実装)。ネスト/composite/配列型カラム、model arguments、native query 前提のモデル | in-scope 23 / pending 77 / out-of-scope 29 | #34, #36 |
