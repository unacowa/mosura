...
FEATURES = [
    ("GraphQL API (クエリ)", [
        ...
        dict(
            feature="モデル select",
            level="partial",
            supported="`where` (スカラー比較 `_eq` `_neq` `_gt` `_gte` `_lt` `_lte` `_in` `_is_null`、"
            "テキスト検索 `_like` / `_nlike` / `_ilike` / `_nilike` / `_similar` / `_nsimilar` / "
            "`_regex` / `_iregex` / `_nregex` / `_niregex`、`_and` / `_or` / `_not`)、"
            "`order_by` (V3 準拠: 方向は `Asc` / `Desc` の 2 値、null 配置固定、"
            "1 エントリ 1 キーで複数キーはエラー)、`limit` / `offset`、"
            "エイリアス・複数ルートフィールド",
            unsupported="ネスト/composite/配列型カラム、model arguments、native query 前提のモデル",
    ...
    ("メタデータ", [
        ...
        dict(
            ...
            "未定義 connector のスタブ生成。Model v1 の orderableFields と v2 の "
            "orderByExpression 参照 (OrderByExpression の object operand) の両対応で、"
            "解決できない参照は fail-closed。未対応要素 (object 表記のフィールド型を持つ "
            "ObjectType 等) はロード全体を落とさず当該定義のみ skip し、参照する Model も "
            "dangling として除外する",
            unsupported="BooleanExpressionType / OrderByExpression の relationship 対応 "
            "(comparableRelationships / orderableRelationships — #32/#38)。"
            "object 表記のフィールド型・globalIdFields・field arguments (#40)、"
            "GraphqlConfig の命名カスタマイズ (既定値のみ使用)、Command 系 Relationship、"
            "rules-based permissions は skip。resolved スナップショット完全一致 (pending)",
